...
The registry is **seeded and live**: a settled invoice, a delinquent one, a bonded
obligation whose bond was forfeited on the mark, and a fabricated debt marked against an
account whose record stays clean. The **Attacks** screen is driven by a real run of the
adversary script against this same registry. Live mode shows all of it; `?fixtures=1`
...
1. **`Attacks`** (`routes/Attacks.tsx`) — the flagship. Three genuine, FDC-confirmed proofs
   that would each do something false, and the contract refusing all three. This is the one
   screen no other team can copy, because the attacks only exist if you went looking for
   them. It renders `src/lib/attacks.json`, regenerated by `services/attester/bin/adversary.js`
   against the live registry. Make it the most quotable page in the app — consider leading
...
2. **`Primitive`** (`routes/Primitive.tsx`) — the conceptual centerpiece: one registry,
   ...
   the side, so "same mechanism, many uses" reads before any words.
3. **`PayInstructions`** — what a judge actually uses. The destination tag has to dominate it.
4. **`BondPanel`** — the stake. Show both futures before the deadline, not just the outcome after.
5. **`Pipeline`** / **`OutcomeAction`** — the ~2 minute attestation wait, made legible.
6. **`ScorePanel`** — the privacy story.
