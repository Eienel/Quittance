...
2. **`PayInstructions`** — what a judge actually uses. The destination tag has to dominate it.
3. **`BondPanel`** — the stake. Show both futures before the deadline, not just the outcome after.
4. **`Pipeline`** / **`OutcomeAction`** — the ~2 minute attestation wait, made legible.
5. **`ScorePanel`** — the privacy story.
